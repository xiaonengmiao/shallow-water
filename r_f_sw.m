...
diffusion(1)=((abslam(1)*lambda(3) - lambda(1)*abslam(3))*diff(1)...
			+ (abslam(3) - abslam(1))*diff(2))/(2.*cavg);
diffusion(2)=(lambda(1)*lambda(3)*(abslam(1)-abslam(3))*diff(1)... 
			+ (lambda(3)*abslam(3)-lambda(1)*abslam(1))*diff(2))/(2.*cavg); 
diffusion(3)=uavg(3)*((lambda(3)*abslam(1)...
			- lambda(1)*abslam(3))/(2.*cavg) - abslam(2))*diff(1)...
			+ uavg(3)*(abslam(3) - abslam(1))/(2.*cavg)+abslam(2)*diff(3);
	
% flux
flux(1,1,1) = .5*(uxl*hl+uxr*hr) - .5*diffusion(1); 
flux(1,1,2) = .5*((uxl*uxl*hl+grav*hl*hl/2.)+(uxr*uxr*hr+grav*hr*hr/2.))... 
			- .5*diffusion(2); 
flux(1,1,3) = .5*(uxl*uyl*hl+uxr*uyr*hr) - .5*diffusion(3);
